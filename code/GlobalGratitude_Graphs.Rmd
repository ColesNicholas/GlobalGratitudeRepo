...
```{r}
# fetch survey
data <- fetch_survey(surveyID = 'SV_1YQ3FmlbEHp1RIO',
                   force_request = T,
                   label = F,
                   convert = F)
...
for (lab_name in unique_labs) {
  ...
  for (i in seq_along(condition_data_list)) {
    ...
    n_condition <- length(condition_data$mean_gratitude)
    
    # Calculate standard error for each measure
    grat_standard_error <- sqrt((grat_var_control / grat_n_control) + (var_condition_grat / n_condition))
    pa_standard_error <- sqrt((pa_var_control / pa_n_control) + (var_condition_pa / n_condition))
    na_standard_error <- sqrt((na_var_control / na_n_control) + (var_condition_na / n_condition))
    ls_standard_error <- sqrt((ls_var_control / ls_n_control) + (var_condition_ls / n_condition))
...
forest_plot <- ggplot(combined_df_final, aes(x = grat_effect_size, y = reorder(lab, grat_effect_size),
                                                     xmin = grat_lower_ci, xmax = grat_upper_ci, color = test)) +
...
# Create the forest plot with custom facet labels (bold test part)
forest_plot <- ggplot(combined_df_final, aes(x = pa_effect_size, y = reorder(lab, pa_effect_size),
                                                     xmin = pa_lower_ci, xmax = pa_upper_ci, color = test)) +
      geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
...
# Create the forest plot with custom facet labels (bold test part)
forest_plot <- ggplot(combined_df_final, aes(x = na_effect_size, y = reorder(lab, na_effect_size),
                                                     xmin = na_lower_ci, xmax = na_upper_ci, color = test)) +
      geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
...
# Create the forest plot with custom facet labels (bold test part)
forest_plot <- ggplot(combined_df_final, aes(x = ls_effect_size, y = reorder(lab, ls_effect_size),
                                                     xmin = ls_lower_ci, xmax = ls_upper_ci, color = test)) +
      geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
