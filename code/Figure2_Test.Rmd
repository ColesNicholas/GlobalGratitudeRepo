...
# Use lapply to apply the operations for each effect size column

results <- lapply(effect_size_columns, function(effect_size) {
  
  # Summarize by country (d.c)
  d.c <- df %>% 
    group_by(country) %>% 
    summarise(m = mean(.data[[effect_size]]), .groups = "drop") %>%
    mutate(
      y = jitter(rep(0, n()), amount = 0.1), 
      label = if_else(m == min(m), true = country, false = ""),
      label = if_else(m == max(m), true = country, false = label),
      effect_size = effect_size 
    )
  
  # Summarize by intervention condition (d.i)
  d.i <- df %>% 
    group_by(intervention_condition) %>% 
    summarise(m = mean(.data[[effect_size]]), .groups = "drop") %>%
    mutate(
      y = jitter(rep(-0.02, n()), amount = 0.1), 
      label = if_else(m == min(m), true = intervention_condition, false = ""),
      label = if_else(m == max(m), true = intervention_condition, false = label),
      effect_size = effect_size
    )
  
  # Return both country-level and intervention-level results as a list
  list(d.c = d.c, d.i = d.i)
})

# Combine results for all effect sizes into separate data frames for country and intervention
d.c <- bind_rows(lapply(results, function(res) res$d.c))
d.i <- bind_rows(lapply(results, function(res) res$d.i))
